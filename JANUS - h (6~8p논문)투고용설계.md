# JANUS-H: 폐쇄망 내 전략적 침투 대응을 위한 계층형 판단 기반 보안 아키텍처 설계 (JANUS-H: A Hierarchical Judgment-Based Security Architecture for Strategic Intrusion Response in Closed Networks)
...
---
| 항목           | 기존 JANUS (3p)                | JANUS-H (확장 구조)                                |
| ------------ | ---------------------------- | ---------------------------------------------- |
| **판단 구조**    | 단일 설계자 (인간 중심 수동 판단)         | **수직 분산 설계자** 구조 (말단\~중앙, 계층적 판단 + 자동화 보조)     |
| **격리 환경**    | Fake DB 기반 샌드박스              | **디지털 트윈 격리 시스템** (더 정밀하고 현실 유사, 유인성 극대화)      |
| **자동화**      | AI 리포트 생성 정도 (보조 수단)         | **LSTM 이상행동 탐지 + 자동 리포트 + GPT 확장 고려**          |
| **로그 구조**    | 일부 정밀 로그, JSON + 파일 + 이메일 백업 | **SHA-256 기반 블록체인 로그** (tamper-proof)          |
| **설계자 위치**   | 유일한 판단 주체 (전지적 설계자)          | **판단자도 KARMA 점수로 평가 대상** (감시/책임 구조 도입)         |
| **Override** | 없음, 설계자 판단이 곧 결정             | **override() 구조 도입**: 상위 설계자가 하위 판단 무효화 가능     |
| **설계자 부재**   | 언급 없음                        | **설계자 부재 시 fail-safe 구조, 임시 격리 + 보고체계 도입**     |
| **시나리오 유형**  | 내부자 위협 1종 중심                 | 내부자/외부자/자동감지/오판/역이용/설계자 판단 오류 등 **다중 시나리오 반영** |
| **추적 방식**    | 정밀 로그 기반                     | **정밀 로그 + 판단 히스토리 누적 + 판단 제안 시스템**             |
| **보안 패러다임**  | 유도-분석-격리-복구                  | **유도 + 행동 위조 유도 + 설계자 판단 감시 + 시뮬레이터 학습 루프**    |
| **철학적 중심**   | 설계자 통제 = 보안 핵심               | 설계자조차 감시받는 **책임 기반 판단 시스템**으로 철학 진화            |
